...
public abstract class BaseScreen<C extends AbstractContainerMenu> extends AbstractContainerScreen<C> {
    ...
    protected void renderLabels(PoseStack poseStack, int x, int y) {
        Minecraft.getInstance().font.draw(poseStack, title, getCenteredOffset(title.getString()), 6, 0x404040);
        Minecraft.getInstance().font.draw(poseStack, playerInventoryTitle, 8, imageHeight - 96 + 3, 0x404040);
    }
    ...
    protected void addSlot(int x, int y, int horizontal, int dx, int vertical, int dy) {
        for (int j = 0; j < vertical; j++) {
            addSlot(x, y, horizontal, dx);
            y += dy;
        }
    }
